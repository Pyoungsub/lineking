<div class="w-full md:w-1/2">
    <h2 class="font-bold text-xl mt-5">요청 예약하기</h2>
    <form wire:submit.prevent="requestingSubstitute" class="w-full md:w-96">
        @csrf
        <div class="relative">
            <x-jet-input id="fullAddress" type="text" class="mt-1 block w-full" wire:model.lazy="fullAddress" autocomplete="off" placeholder="위치입력" />
            @if($trigger<>null)
        ...
        <x-jet-input id="detailedAddress" type="text" class="mt-1 block w-full" wire:model.lazy="detailedAddress" autocomplete="off" placeholder="세부장소" />
        <x-jet-input-error for="detailedAddress" class="mt-2" />
        <div class="sm:flex">
            <script src="https://cdn.jsdelivr.net/npm/pikaday/pikaday.js"></script>
            <link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/npm/pikaday/css/pikaday.css">
            <div class="block">
                <x-jet-input id="reservedDate" type="text" class="mt-1 w-full" wire:model.lazy="reservedDate" autocomplete="off" placeholder="예약날짜" />
            ...
            <div class="relative">
                <x-jet-dropdown align="left" width="48">
                    <x-slot name="trigger">
                        <x-jet-input id="reservedTime" type="text" class="mt-1 w-full static" wire:click="$toggle('timeList')" wire:model.lazy="reservedTime" autocomplete="off" placeholder="예약시간" />
                    </x-slot>
                    <x-slot name="content">
        ...
        <div class="relative">
            <x-jet-input id="maximumCost" type="text" class="mt-1 block w-full" wire:model="maximumCost" autocomplete="off" placeholder="최대금액" />
            <i class="fas fa-won-sign fa-sm absolute top-0 right-0" style="padding:12px"></i>
        </div>
        <x-jet-input-error for="maximumCost" class="mt-2" />
        <textarea id="detailedMessage" wire:model.lazy="detailedMessage" style="resize:none" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" rows="6" placeholder="추가사항(확인)"></textarea>
