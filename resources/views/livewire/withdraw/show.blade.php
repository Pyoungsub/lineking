<div>
    <div class="max-w-6xl py-10 mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex">
            <div wire:click="typeChange('request')" class="{{$type == 'request' ? 'text-white bg-gray-800': 'bg-gray-200'}} rounded-full px-4 py-2 mr-4 cursor-pointer">
                출금신청
            </div>
            <div wire:click="typeChange('history')" class="{{$type == 'history' ? 'text-white bg-gray-800': 'bg-gray-200'}} rounded-full px-4 py-2 cursor-pointer">
                출금 내역
            </div>
        </div>
    </div>
    ...
    <div class="max-w-6xl py-10 mx-auto sm:px-6 lg:px-8">
        @if($type == 'request')
            @livewire('withdraw.request')
        @elseif($type == 'history')
            @livewire('withdraw.history')
        @endif
    </div>
</div>
