        ...
        @if(count($results)>0)
            @foreach($results as $result)
                <div class="p-4 flex" wire:click="chat({{$result->id}})">
                        ...
                        <div class="ml-2 w-full">
                            <div class="flex justify-between">
                                ...
                                <div class="text-gray-500">
                                    @if(count($result->chatHistory)>0)
                                        {{$result->chatHistory->last()->created_at->diffForHumans()}}
                                    @endif
                                </div>
                            </div>
                            <div class="text-gray-500">
                                @if(count($result->chatHistory)>0)
                                    {{$result->chatHistory->last()->message}}
                                @else
                                    대화 시작하기
                                @endif
                            </div>
                    ...
                    @else
                        <img class="mt-1 h-10 w-10 rounded-full object-cover" src="{{$result->selectedApplicant->user->profile_photo_url}}" alt="{{$result->selectedApplicant->user->name}}">
                        <div class="ml-2 w-full">
                            <div class="flex justify-between">
                                <div>
                                    {{$result->selectedApplicant->user->name}}
                                </div>
                                <div class="text-gray-500">
                                    @if(count($result->chatHistory)>0)
                                        {{$result->chatHistory->last()->created_at->diffForHumans()}}
